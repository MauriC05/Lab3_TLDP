...
PROJECT_ID = "tldp-etl-lab-488501"  # Replace with your GCP project ID
OUTPUT_DIR = 'output'
...
def extract(client: bigquery.Client) -> pd.DataFrame:
    ...
    df = client.query(query).to_dataframe()  # runs a query and saves it as df
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['engagement_score'] = round((df['avg_score'] * 0.4 + df['avg_views'] / 1000 * 0.4 + df['avg_answers'] * 0.2), 2)
    ...
    #print(df[['tag', 'total_questions','engagement_score','unanswered_rate']].head(10))
